...
# Use temp directory for uploads in production
...
def search_company_website(company_name):
    """Search for company website using DuckDuckGo search"""
    try:
        if not company_name:
            return None
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'message': 'FashionGo Email Scraper is running'}), 200
...
def upload_file():
    try:
        ...
        # Process companies (limit to first 10 for demo to avoid timeouts)
        results = []
        total_companies = min(len(df), 10)  # Limit for demo
        ...
        for index, row in df.head(total_companies).iterrows():
            try:
                ...
                result_row['processed_company_name'] = str(company_name_val) if not pd.isna(company_name_val) else 'N/A'
                results.append(result_row)
                continue
        ...
        # Save results to CSV in temp directory
        ...
        return jsonify({
            ...
            'note': 'Demo version processes first 10 companies only'
        })
